...
The default/full installer includes the dashboard dependencies. Compact/minimal installs do not ship the HTTP stack or PTY helper unless you add them. The **web dashboard** needs FastAPI and Uvicorn (public `dashboard` feature). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
pip install 'hermes-agent[dashboard,pty]'
```

The `dashboard` feature pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `pip install hermes-agent[all]` includes both extras and is the easiest path if you also want messaging/voice/etc. The old `web` extra remains as a hidden backwards-compatible alias for dashboard deps; use `dashboard` in new docs and support instructions.
...
- `ptyprocess` — installed by the `pty` extra (`pip install 'hermes-agent[dashboard,pty]'`, or `[all]` covers both)
